...
export class Rectangle extends Shape {
  ...
  translate(dx: Measurement, dy: Measurement): Rectangle {
    ...
    const result = new Rectangle(this.topLeft.translate(dx, dy), this.width, this.height);
  ...
  rotate(angle: Angle, origin: Point): Rectangle {
    ...
    const result = utils.boundingBoxFromPoints(corners) as Rectangle;
    this.copyStyleTo(result);
    return result;
  }
  ...
  scale(factor: number, origin: Point): Rectangle {
    const result = new Rectangle(
      utils.scalePoint(this.topLeft, factor, origin),
