...
describe('CanvasTabComponent (modern event & geometry)', () => {
  ...
  function setupComponent() {
    ...
    const persistence = {
      ...
      deserializeShapes: (d: any[]) => Promise.resolve([]),
  ...
  it('pans viewport with pointer drag', () => {
    ...
    const move = new PointerEvent('pointermove', { clientX: 30, clientY: 25, pointerId, buttons: 4 });
  ...
  it('selects a shape on click', () => {
    ...
    expect((comp as any).selectionController.model.selectedShapes[0]).toBe(rect);
  });
  ...
  it('toggles selection with shift+click', () => {
    ...
    expect((comp as any).selectionController.model.selectedShapes).toEqual([rect]);
    // Shift+click circ
    canvas.dispatchEvent(new MouseEvent('click', { clientX: 80, clientY: 80, shiftKey: true }));
    expect((comp as any).selectionController.model.selectedShapes).toContain(rect);
    expect((comp as any).selectionController.model.selectedShapes).toContain(circ);
    // Shift+click rect again to deselect
    canvas.dispatchEvent(new MouseEvent('click', { clientX: 20, clientY: 20, shiftKey: true }));
    expect((comp as any).selectionController.model.selectedShapes).toEqual([circ]);
  });
  ...
  it('drag-selects multiple shapes', () => {
    ...
    canvas.dispatchEvent(new PointerEvent('pointermove', { clientX: 120, clientY: 100, pointerId, buttons: 1 }));
  ...
  it('moves selected shape with drag', () => {
    ...
    expect((comp as any).selectionController.model.selectedShapes.length).toBe(1);
    const selected = (comp as any).selectionController.model.selectedShapes[0];
  ...
  it('getGroupBoundingBox returns correct bbox for selection', () => {
    ...
    const circ = new Circle(new Point(Measurement.fromPx(80), Measurement.fromPx(80)), Measurement.fromPx(15));
    comp.shapes = [rect, circ];
    (comp as any).selectionController.model.selectedShapes = [rect, circ];
