...
constructor(readonly model: CanvasSelectionModel) {}

  /** Pointer down on a shape */
  pointerDownOnShape(hit: Shape, shift: boolean): boolean {
    if (shift) return this.model.apply({ type: 'toggle', shapes: [hit] });

    if (!this.model.isSelected(hit)) {
      return this.model.apply({ type: 'replace', shapes: [hit] });
    }
    return false;
  }
  ...
  pointerDownOnEmpty(shift: boolean): boolean {
    return shift ? false : this.model.apply({ type: 'replace', shapes: [] });
  }

  /** Click on shape */
  clickOnShape(hit: Shape, shift: boolean): boolean {
  ...
  commitMarquee(selected: Shape[], shift: boolean): boolean {
    return this.model.apply({ type: shift ? 'add' : 'replace', shapes: selected });
  }
  ...
  /** Restore a previously snapshotted selection (used by undo/redo). */
  replaceSelection(shapes: Shape[]): void {
    this.model.selectedShapes = [...shapes];
  }
