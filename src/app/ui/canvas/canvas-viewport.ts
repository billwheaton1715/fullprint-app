...
export class CanvasViewport {
 ...
 /**
  ...
  /**
   * Fit the viewport so that the given world-space bounding box (in px) fills
   * the canvas (screenW × screenH), with uniform padding on all sides.
   * padding is a fraction of the smaller canvas dimension (default 5%).
   */
  public fitToRect(
    minXpx: number, minYpx: number,
    maxXpx: number, maxYpx: number,
    screenW: number, screenH: number,
    padding = 0.05
  ): void {
    const bw = maxXpx - minXpx;
    const bh = maxYpx - minYpx;
    if (bw <= 0 || bh <= 0 || screenW <= 0 || screenH <= 0) return;

    const paddingPx = Math.min(screenW, screenH) * padding;
    const availW = screenW - paddingPx * 2;
    const availH = screenH - paddingPx * 2;

    const newScale = Math.min(availW / bw, availH / bh);
    const centerX = (minXpx + maxXpx) / 2;
    const centerY = (minYpx + maxYpx) / 2;

    this.scale   = newScale;
    this.offsetX = screenW / 2 - centerX * newScale;
    this.offsetY = screenH / 2 - centerY * newScale;
  }
}
