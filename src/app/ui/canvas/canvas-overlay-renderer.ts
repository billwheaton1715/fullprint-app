...
import { SnapGuide } from './canvas-snap-controller';
...
export interface CanvasOverlayState {
  ...
  /** Snap guide lines drawn during a drag when snapping fires. */
...
export class CanvasOverlayRenderer {
  draw(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, state: CanvasOverlayState) {
    ...
    if (shapesForOverlay && shapesForOverlay.length) {
      ...
      // Per-shape selected outlines — stroke only, no fill, so the ink-saver
      // effect on the canvas is not overwritten.
      ctx.save();
      ...
      state.selectedIndices.forEach(i => {
        ...
        try {
          ...
          ctx.strokeRect(
            bb.topLeft.x.toUnit('px'), bb.topLeft.y.toUnit('px'),
            bb.width.toUnit('px'),     bb.height.toUnit('px'),
          );
        } catch {}
      });
    ...
    if (isHoveringSelectable && state.hoveredShape) {
      ...
      ctx.lineWidth = 1 / (viewport.getScale() || 1);
      try {
        const bb = state.hoveredShape.getBoundingBox();
        ctx.strokeRect(
          bb.topLeft.x.toUnit('px'), bb.topLeft.y.toUnit('px'),
          bb.width.toUnit('px'),     bb.height.toUnit('px'),
        );
    ...
    if (state.snapGuides && state.snapGuides.length > 0) {
      ctx.save();
      ctx.strokeStyle = 'rgba(255, 0, 200, 0.75)';
      ctx.lineWidth = 1 / (viewport.getScale() || 1);
      ctx.setLineDash([]);
      ctx.beginPath();
      for (const g of state.snapGuides) {
        if (g.axis === 'x') {
          // vertical line
          ctx.moveTo(g.worldCoord, topLeft ? topLeft.yPx - 10000 : -10000);
          ctx.lineTo(g.worldCoord, bottomRight ? bottomRight.yPx + 10000 : 10000);
        } else {
          // horizontal line
          ctx.moveTo(topLeft ? topLeft.xPx - 10000 : -10000, g.worldCoord);
          ctx.lineTo(bottomRight ? bottomRight.xPx + 10000 : 10000, g.worldCoord);
        }
      }
      ctx.stroke();
      ctx.restore();
